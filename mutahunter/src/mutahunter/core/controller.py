...
MUTANT_RESULTS_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),"report/mutant_status.json")
class MutationTestController:
    ...
    def test_mutant(
        ...
        except (MutantSurvivedError, MutantKilledError) as e:
            mutant_status = {
                "file": os.path.basename(mutant_path),
                "status": "SURVIVED" if isinstance(e, MutantSurvivedError) else "KILLED"
            }
            self.save_mutant_result(mutant_status)
            raise e
       

        

    def save_mutant_result(self, status_entry: Dict[str, str]) -> None:
    # Create the report folder if it doesn't exist
        os.makedirs(os.path.dirname(MUTANT_RESULTS_PATH), exist_ok=True)
        
        try:
            if os.path.exists(MUTANT_RESULTS_PATH):
                with open(MUTANT_RESULTS_PATH, "r") as f:
                    data = json.load(f)
            else:
                data = []

            data.append(status_entry)

            with open(MUTANT_RESULTS_PATH, "w") as f:
                json.dump(data, f, indent=4)

        except Exception as e:
            logger.error(f"Failed to save mutant result: {str(e)}")
